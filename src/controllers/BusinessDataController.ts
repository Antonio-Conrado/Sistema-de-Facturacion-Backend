...
export class BusinessDataController {
    static getBusinessData = async (req: Request, res: Response) => {
        try {
            ...
            if (businessData) {
                res.status(404).json({ error: 'Los datos del negocio no existen' })
                return
            }
    ...
    static updateBusinessData = async (req: Request, res: Response) => {
        try {
            ...
            if (existingRecord) {
                ...
                res.status(200).json({ msg: 'Datos del negocio actualizados exitosamente' });
            } else {
                await prisma.businessData.create({
                    data: req.body,
                });
                res.status(200).json({ msg: 'Datos del negocio creados exitosamente' });
            }
        } catch (error) {
            res.status(500).json({ error: 'Hubo un error' })
        }
    ...
    static uploadImage = async (req: Request, res: Response) => {
        try {
            ...
            if (!businessData) {
                res.status(404).json({ error: 'No hay datos disponibles del negocio' })
                return
            }

...
export class AuthController {
    ...
    static login = async (req: Request, res: Response) => {
        try {
            ...
            const password = await checkPassword(req.body.password, user.password)
            if (!password) {
    ...
    static forgotPassword = async (req: Request, res: Response) => {
        ...
        try {
            ...
            if (!user.isConfirm) {
                res.status(404).json({ error: "La cuenta no ha sído confirmada" })
                return
            }
            user.token = generateToken()
            ...
            res.status(200).send('Revisa tu email para restablecer tu password')
        } catch (error) {
            res.status(500).json({ error: 'Hubo un error' })
        }
    }
    ...
    static validateToken = async (req: Request, res: Response) => {
        ...
        try {
            const tokenExists = await prisma.users.findUnique({ where: { token } })
            if (!tokenExists) {

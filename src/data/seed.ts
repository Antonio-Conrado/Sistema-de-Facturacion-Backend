import * as bcrypt from 'bcrypt';
import { Role } from '../types';
import { PaymentMethods } from './data';

export const basicSeed = {
    roles: [
        { name: Role.admin, status: true },
        { name: Role.employee, status: true },
    ],
    users: [
        {
            name: 'John',
            surname: 'Doe',
            telephone: '12345678',
            email: 'correo@correo.com',
            password: bcrypt.hashSync('Abc123', 10),
            isConfirm: true,
            status: true,
            roleId: 1,
        },
        {
            name: 'Nicole',
            surname: 'Vach',
            telephone: '12345678',
            email: 'correo1@correo.com',
            password: bcrypt.hashSync('Abc123', 10),
            isConfirm: true,
            status: true,
            roleId: 2,
        },
    ],
    suppliers: [
        {
            ruc: '1234567890123',
            name: 'Proveedor General',
            direction: 'Dirección del proveedor',
            telephone: '987654321',
            email: 'proveedor@correo.com',
            status: true,
        },
    ],
    paymentMethods: [
        { name: PaymentMethods.cash, status: true },
        { name: PaymentMethods.bankTransfer, status: true },
    ],
    businessData: [
        {
            ruc: '9876543210987',
            name: 'nicafact',
            description: 'Descripción del negocio',
            direction: 'Dirección de negocio',
            telephone: '111222333',
            email: 'contacto@correo.com',
            image: 'https://res.cloudinary.com/dqh2rff6j/image/upload/v1758722653/d210b44b-6a42-4e84-9635-6b2688d99766.jpg',
        },
    ],
};

export const fullSeed = {
    roles: [
        { name: Role.admin, status: true },
        { name: Role.employee, status: true },
    ],
    users: [
        {
            name: 'John',
            surname: 'Doe',
            telephone: '12345678',
            email: 'correo@correo.com',
            password: bcrypt.hashSync('Abc123', 10),
            isConfirm: true,
            status: true,
            roleId: 1,
        },
        {
            name: 'Nicole',
            surname: 'Vach',
            telephone: '12345678',
            email: 'correo1@correo.com',
            password: bcrypt.hashSync('Abc123', 10),
            isConfirm: true,
            status: true,
            roleId: 2,
        },
    ],
    suppliers: [
        {
            ruc: '1234567890123',
            name: 'Proveedor General',
            direction: 'Dirección del proveedor',
            telephone: '97654321',
            email: 'proveedor@correo.com',
            status: true,
        },
        {
            ruc: '12345671222',
            name: 'Proveedor Secundario',
            direction: 'Dirección del proveedor',
            telephone: '97224321',
            email: 'proveedor2@correo.com',
            status: true,
        },
    ],
    paymentMethods: [
        { name: PaymentMethods.cash, status: true },
        { name: PaymentMethods.bankTransfer, status: true },
    ],
    businessData: [
        {
            ruc: '9876543210987',
            name: 'nicafact',
            description: 'Descripción del negocio',
            direction: 'Dirección de negocio',
            telephone: '111222333',
            email: 'contacto@correo.com',
            image: 'https://res.cloudinary.com/dqh2rff6j/image/upload/v1758722653/d210b44b-6a42-4e84-9635-6b2688d99766.jpg',
        },
    ],
    categories: [
        {
            name: 'Electrónica',
            description: 'Productos electrónicos y dispositivos',
            status: true,
        },
        {
            name: 'Muebles',
            description: 'Muebles para hogares y oficinas',
            status: true,
        },
        {
            name: 'Ropa',
            description: 'Prendas de vestir y vestimenta',
            status: true,
        },
    ],
    products: [
        {
            code: 'P001',
            name: 'Teléfono inteligente',
            categoriesId: 1,
            status: true,
        },
        {
            code: 'P002',
            name: 'Silla de oficina',
            categoriesId: 2,
            status: true,
        },
        { code: 'P003', name: 'Camiseta', categoriesId: 3, status: true },
    ],

    detailsProducts: [
        {
            productsId: 1,
            description: 'Teléfono inteligente de última generación',
            image: 'https://imgs.search.brave.com/QWIfW7hmRAoPaQq2UmfKSGRI_tOtndZExXFpP7tbNLQ/rs:fit:860:0:0:0/g:ce/aHR0cHM6Ly9odHRw/Mi5tbHN0YXRpYy5j/b20vRF82MTI4NTgt/TUxNNzk1NjA5MDc3/OTRfMTAyMDI0LU8t/c2Ftc3VuZy1nYWxh/eHktczI0LWZlLWR1/YWwtMjU2Z2ItOHJh/bS1taW50LndlYnA',
        },
        {
            productsId: 2,
            description: 'Silla de oficina ergonómica',
            image: 'https://imgs.search.brave.com/9dkP2UgyQVUkiFcqJ3eciUrAYhcnTUouudyZhjGxdsY/rs:fit:860:0:0:0/g:ce/aHR0cHM6Ly9pbWcu/ZnJlZXBpay5jb20v/ZnJlZS1waG90by9z/dGlsbC1saWZlLW9m/ZmljZS1jaGFpci1p/bmRvb3JzXzIzLTIx/NTExMDg3MjguanBn/P3NlbXQ9YWlzX2h5/YnJpZCZ3PTc0MA',
        },
        {
            productsId: 3,
            description: 'Camiseta 100% de algodón',
            image: 'https://imgs.search.brave.com/onzM7M_rW0DJvbrKLScyZBeYTX7HhUay4bTfLviXr5c/rs:fit:860:0:0:0/g:ce/aHR0cHM6Ly9jZG4u/cGl4YWJheS5jb20v/cGhvdG8vMjAyNC8w/NC8xNy8xOC80MC9h/aS1nZW5lcmF0ZWQt/ODcwMjcyNl82NDAu/anBn',
        },
    ],
    storedProducts: [
        {
            detailsProductsId: 1,
            stock: 100,
            purchasePrice: 5200,
            salePrice: 6200,
            status: true,
        },
        {
            detailsProductsId: 2,
            stock: 50,
            purchasePrice: 8000,
            salePrice: 9000,
            status: true,
        },
        {
            detailsProductsId: 3,
            stock: 200,
            purchasePrice: 200,
            salePrice: 320,
            status: true,
        },
    ],
    purchases: [
        {
            usersId: 1,
            suppliersId: 1,
            iva: 15,
            invoiceNumber: 1001,
            date: new Date('2025-06-30T10:00:00Z'),
            subtotal: 92000,
            discount: 20,
            total: 103500,
            status: true,
        },
        {
            usersId: 2,
            suppliersId: 2,
            iva: 15,
            invoiceNumber: 1002,
            date: new Date('2025-07-30T11:00:00Z'),
            subtotal: 40000,
            discount: 0,
            total: 46000,
            status: true,
        },
        {
            usersId: 1,
            suppliersId: 2,
            iva: 15,
            invoiceNumber: 1004,
            date: new Date(),
            subtotal: 80000,
            discount: 0,
            total: 92000,
            status: true,
        },
    ],
    detailsPurchases: [
        {
            purchasesId: 1,
            storedProductsId: 1,
            amount: 10,
            purchasePrice: 5200,
            salePrice: 6200,
            discount: 0,
            subtotal: 52000,
        },
        {
            purchasesId: 1,
            storedProductsId: 2,
            amount: 5,
            purchasePrice: 8000,
            salePrice: 9000,
            discount: 20,
            subtotal: 40000,
        },
        {
            purchasesId: 2,
            storedProductsId: 3,
            amount: 200,
            purchasePrice: 200,
            salePrice: 320,
            discount: 0,
            subtotal: 40000,
        },
        {
            purchasesId: 3,
            storedProductsId: 2,
            amount: 10,
            purchasePrice: 8000,
            salePrice: 9000,
            discount: 0,
            subtotal: 80000,
        },
    ],
    sales: [
        {
            usersId: 1,
            paymentMethodId: 1,
            iva: 15,
            transactionReference: 'TX123456',
            invoiceNumber: 5001,
            date: new Date('2025-07-01T10:00:00Z'),
            subtotal: 70500,
            discount: 20,
            total: 78675,
            status: true,
        },
        {
            usersId: 2,
            paymentMethodId: 2,
            iva: 15,
            transactionReference: 'TX123457',
            invoiceNumber: 5002,
            date: new Date('2025-07-01T11:00:00Z'),
            subtotal: 47300,
            discount: 0,
            total: 54395,
            status: true,
        },
        {
            usersId: 2,
            paymentMethodId: 2,
            iva: 15,
            transactionReference: 'TX42457',
            invoiceNumber: 5003,
            date: new Date('2025-08-01T11:00:00Z'),
            subtotal: 4800,
            discount: 0,
            total: 5520,
            status: true,
        },
        {
            usersId: 2,
            paymentMethodId: 2,
            iva: 15,
            transactionReference: 'TX42aa457',
            invoiceNumber: 5004,
            date: new Date('2025-08-01T11:00:00Z'),
            subtotal: 39840,
            discount: 0,
            total: 45816,
            status: true,
        },
        {
            usersId: 2,
            paymentMethodId: 1,
            iva: 15,
            transactionReference: 'TX999999',
            invoiceNumber: 5005,
            date: new Date(),
            subtotal: 12000, // venta hoy
            discount: 0,
            total: 13800, // con IVA
            status: true,
        },
        {
            usersId: 1,
            paymentMethodId: 1,
            iva: 15,
            transactionReference: 'TX555555',
            invoiceNumber: 5006,
            date: new Date(),
            subtotal: 50000,
            discount: 0,
            total: 57500, // subtotal + 15% IVA
            status: true,
        },
        {
            usersId: 2,
            paymentMethodId: 2,
            iva: 15,
            transactionReference: 'TX666666',
            invoiceNumber: 5007,
            date: new Date(),
            subtotal: 50000,
            discount: 0,
            total: 57500,
            status: true,
        },
        {
            usersId: 1,
            paymentMethodId: 1,
            iva: 15,
            transactionReference: 'TX777777',
            invoiceNumber: 5008,
            date: new Date(),
            subtotal: 25000,
            discount: 0,
            total: 28750, // subtotal + 15% IVA
            status: true,
        },
        {
            usersId: 1,
            paymentMethodId: 2,
            iva: 15,
            transactionReference: 'TX888888',
            invoiceNumber: 5009,
            date: new Date(),
            subtotal: 15000,
            discount: 0,
            total: 17250, // subtotal + 15% IVA
            status: true,
        },
    ],
    detailsSales: [
        {
            salesId: 1,
            storedProductsId: 1,
            price: 6200,
            amount: 10,
            subtotal: 62000,
            discount: 0,
        },
        {
            salesId: 1,
            storedProductsId: 2,
            price: 8500,
            amount: 1,
            subtotal: 8500,
            discount: 2000,
        },
        {
            salesId: 2,
            storedProductsId: 2,
            price: 8500,
            amount: 5,
            subtotal: 42500,
            discount: 0,
        },
        {
            salesId: 2,
            storedProductsId: 3,
            price: 320,
            amount: 15,
            subtotal: 4800,
            discount: 0,
        },
        {
            salesId: 3,
            storedProductsId: 3,
            price: 320,
            amount: 15,
            subtotal: 4800,
            discount: 0,
        },
        {
            salesId: 4,
            storedProductsId: 2,
            price: 9000,
            amount: 4,
            subtotal: 36000,
            discount: 0,
        },
        {
            salesId: 4,
            storedProductsId: 3,
            price: 320,
            amount: 12,
            subtotal: 3840,
            discount: 0,
        },

        {
            salesId: 4,
            storedProductsId: 2,
            price: 11000,
            amount: 1,
            subtotal: 11000,
            discount: 0,
        }, // venta hoy
        {
            salesId: 5,
            storedProductsId: 1,
            price: 6200,
            amount: 5,
            subtotal: 31000,
            discount: 0,
        },
        {
            salesId: 5,
            storedProductsId: 3,
            price: 320,
            amount: 60,
            subtotal: 19200,
            discount: 0,
        },
        {
            salesId: 6,
            storedProductsId: 2,
            price: 9000,
            amount: 5,
            subtotal: 45000,
            discount: 0,
        },
        {
            salesId: 6,
            storedProductsId: 3,
            price: 320,
            amount: 39,
            subtotal: 12480,
            discount: 0,
        },
        {
            salesId: 7,
            storedProductsId: 1,
            price: 6200,
            amount: 2,
            subtotal: 12400,
            discount: 0,
        },
        {
            salesId: 7,
            storedProductsId: 3,
            price: 320,
            amount: 40,
            subtotal: 12800,
            discount: 0,
        },
        {
            salesId: 8,
            storedProductsId: 1,
            price: 6200,
            amount: 2,
            subtotal: 12400,
            discount: 0,
        }, // Smartphone
        {
            salesId: 8,
            storedProductsId: 3,
            price: 320,
            amount: 8,
            subtotal: 2560,
            discount: 0,
        }, // T-Shirt
    ],
};
